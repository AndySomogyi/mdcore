...
int engine_bonded_sets ( struct engine *e , int max_sets ) {
	...
	while ( nr_sets > max_sets ) {
		...
		// avg_nconfl = (2.0 * bs.confl_count) / nr_sets;
		...
		/* printf( "engine_bonded_sets: nr_sets=%i, bs.confl_count=%i, avg_weight=%f, avg_nconfl=%f.\n" ,
            nr_sets, bs.confl_count, avg_weight , avg_nconfl );
        fflush(stdout); */
		...
		if ( min_i < nr_sets && min_j < nr_sets ) {
			...
			/* printf( "engine_bonded_sets: found disjoint sets %i and %i, %i confl.\n" ,
                min_i , min_j , bs.nconfl[min_i] + bs.nconfl[min_j] ); */
		...
		/* Otherwise, look for a pair sharing a conflict. */
            ...
            for ( avg_nconfl = 0 , k = 0 ; k < nr_sets ; k++ )
                avg_nconfl += bs.nconfl[k];
            if ( avg_nconfl/2 != bs.confl_count )
                printf( "engine_bonded_sets: inconsistent bs.nconfl (%f != %i)!\n" ,
                    avg_nconfl/2 , bs.confl_count );
            for ( k = 1 ; k < 2*bs.confl_count ; k++ )
                if ( bs.confl_sorted[k].i < bs.confl_sorted[k-1].i )
                    printf( "engine_bonded_sets: sorting is botched!\n" );
            for ( k = 0 ; k < nr_sets ; k++ )
                if ( confl_index[k+1]-confl_index[k] != bs.nconfl[k] ) {
                    printf( "engine_bonded_sets: bs.nconfl and confl inconsistent (%i:%i-%i != %i)!\n" ,
                        k , confl_index[k+1] , confl_index[k] , bs.nconfl[k] );
                    printf( "engine_bonded_sets: conflicts are" );
                    for ( j = confl_index[k] ; j < confl_index[k+1] ; j++ )
                        printf( " [%i,%i]" , bs.confl_sorted[j].i , bs.confl_sorted[j].j );
                    printf( ".\n" );
                    } */
		...
		/* printf( "engine_bonded_sets: merged sets %i and %i, weight %i and %i confl.\n" ,
            min_i , min_j , weight[min_i] , bs.nconfl[min_i] ); fflush(stdout);
        getchar(); */
